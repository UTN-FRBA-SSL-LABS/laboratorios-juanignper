...
> R: Inicializa el valor de cantidad en 0. Porque sino el carrito tendria un numero no definido al momento de agregar un item.
...
> R: 24, el total del carrito es igual a la sumatoria de los precios de los items. 

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Deberia multiplicar la cantidad por el precio de cada item.
...
> R: Falla, el 5to carrito devuelve 1 en vez de 0
...
> R:Si, las lineas 29 y 30. No hay tests que validen esa funcion.
...
> R:El test unitario valida funciones por si solas y el test de integracion valida las funciones juntas, un proceso completo. Los test unitarios la descubrieron primero.
...
> R: Es peligroso en C porque el lenguaje permite el acceso directo a la memoria sin verificar los límites de los arrays. Al escribir más allá del espacio asignado (buffer overflow), se pueden sobrescribir datos adyacentes en la memoria, lo que puede corromper el estado del programa, causar fallos impredecibles o, peor aún, ser explotado para ejecutar código malicioso. En lenguajes como Python o Java, esto no ocurre porque su entorno de ejecución gestiona la memoria y realiza comprobaciones de límites (bounds checking) en cada acceso. Si se intenta acceder a un índice inválido, el programa se detiene de forma segura lanzando una excepción, en lugar de corromper la memoria.
...
> R: Permite encontrar errores que a simple vista no se ven. Normalmente vemos errores en la sintaxis pero los tests nos permiten ver errores en la semantica del programa, si verdaderamente resuelven el problema que dicen resolver.
...
> R: El primer test no detectó el bug porque, para el caso específico de `cantidad = 1`, el cálculo erróneo (`total += item.precio`) y el cálculo correcto (`total += item.precio * item.cantidad`) producen el mismo resultado. El bug solo se manifiesta cuando la cantidad es distinta de 1, que es precisamente lo que el segundo test verificó.
